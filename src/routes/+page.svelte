<script>
  ...
  const homeSeoTitle = 'Michał Danieluk — SEO, reklamy i marketing od praktyka'
  const homeMetaDescription =
    'Praktyczne poradniki o SEO, Google Ads, Meta Ads i marketingu małej firmy. Konkretne testy, koszty i decyzje — bez agencyjnej waty.'
...
<section class="relative overflow-hidden pt-12 pb-20 sm:pt-16 sm:pb-24">
  <div class="flex flex-col items-center text-center gap-8 px-4 max-w-4xl mx-auto">
    <div class="relative">
      <div
        class="absolute inset-0 rounded-full bg-indigo-500/20 blur-2xl dark:bg-indigo-400/10"
      ></div>
      <img
        src={avatar}
        alt={name}
    ...
    <div class="space-y-4">
      <h1
        class="text-3xl sm:text-5xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50"
      >
        Cześć, jestem <span class="text-indigo-600 dark:text-indigo-400">{name.split(' ')[0]}</span
        >.
      </h1>
      <p
        class="text-lg sm:text-xl text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto leading-relaxed"
      >
        {bio}
      </p>
      <nav aria-label="Główne tematy" class="flex flex-wrap justify-center gap-2 pt-2">
        <a
          href="/seo"
          class="rounded-full border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-700 transition-colors hover:border-indigo-500 hover:text-indigo-600 dark:border-zinc-800 dark:text-zinc-300 dark:hover:border-indigo-400 dark:hover:text-indigo-400"
          >SEO</a
        >
        <a
          href="/google-ads"
...
<div class="flex flex-col flex-grow pb-24 space-y-24">
  ...
  {#if data.featuredPosts && data.featuredPosts.length > 0}
    <section class="relative">
      <div
        class="absolute inset-0 -top-12 -bottom-12 bg-zinc-100/50 dark:bg-zinc-900/50 -skew-y-1"
      ></div>
  ...
  <section class="container mx-auto px-4 max-w-4xl">
    <div class="flex items-center justify-between gap-4 mb-10">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-xl font-bold text-zinc-900 dark:text-zinc-50">Najnowsze wpisy</h2>
      </div>
      <a
        href="/posts"
        class="group flex items-center gap-1 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition-colors"
