...
<div class="max-w-4xl mx-auto px-6 pt-12 lg:pt-20 pb-24">
  <header class="mb-16">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end gap-6 mb-8">
      <div class="space-y-4">
        <div class="flex items-center gap-3">
          ...
          <span
            ...
            >Filtrowanie według tematu</span
          >
        </div>
        <h1
          class="text-4xl sm:text-6xl font-black tracking-tight text-zinc-900 dark:text-zinc-50 leading-none"
        >
          {data.tag.name}
        </h1>
      </div>
      ...
      <div class="shrink-0 mb-1">
        <span
          class="inline-flex items-center px-4 py-2 text-xs font-black bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-2xl uppercase tracking-widest"
        >
          {data.tag.count}
          {data.tag.count === 1 ? 'Wpis' : data.tag.count < 5 ? 'Wpisy' : 'Wpisów'}
        </span>
      </div>
    </div>

    <p class="text-lg text-zinc-600 dark:text-zinc-400 max-w-2xl leading-relaxed">
      Wszystkie artykuły i przemyślenia powiązane z tematem: <span
        class="text-zinc-900 dark:text-zinc-100 font-bold">"{data.tag.name}"</span
      >.
    </p>
  </header>
  ...
  <div class="mt-12">
    ...
    {:else}
      <div
        class="bg-zinc-100 dark:bg-zinc-900/50 rounded-2xl p-20 text-center border border-dashed border-zinc-300 dark:border-zinc-800"
      >
