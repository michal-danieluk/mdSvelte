...
>
  ...
  <kbd
    class="hidden lg:inline-flex items-center gap-1 px-1.5 py-0.5 rounded border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 text-[10px] font-black text-zinc-400"
  >
...
{#if isOpen}
  ...
  >
    ...
    >
      ...
      <div class="max-h-[60vh] overflow-y-auto p-2">
        {#if query.length > 1}
          {#if filteredPosts.length > 0}
            <div
              class="p-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500"
            >
            ...
            <div class="grid gap-1 mt-1">
              {#each filteredPosts as post}
                ...
                >
                  <span
                    class="text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-1"
                    >{post.date}</span
                  >
                  <span
                    class="text-sm font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                  >
                    {post.title}
                  </span>
                </a>
      ...
      <div
        class="p-4 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center"
      >
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-1.5">
            <kbd
              class="px-1.5 py-0.5 rounded border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-[10px] font-black text-zinc-500"
              >ESC</kbd
            >
        ...
        <span class="text-[10px] font-black text-indigo-500/50 uppercase tracking-widest"
          >MD.BLOG Search v1</span
        >
      </div>
    </div>
  </div>
